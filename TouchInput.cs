    ...
    void Update()
    {
        ...
        if (allowedToMove)
        {
            ...
            else if (Input.anyKey)
            {
                ...
                if (Mathf.Abs(poitsukkaPos.transform.position.x - checkpoints[destMod].transform.position.x) < 1 && Mathf.Abs(poitsukkaPos.transform.position.y - checkpoints[destMod].transform.position.y) < 1)
                {
