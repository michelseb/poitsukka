...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (currentInteractable)
        {
            ...
            distToArrival = Vector2.Distance(transform.position, destinationPosition);//Mathf.Abs(transform.position.x-destinationPosition.x);
        }

        if (hasToGo == true && distToArrival > 2.3f)
        {
            Moving();
        }
        else if (hasToGo == true && distToArrival <= 2.3f)
        {
            Stopping();
        }
